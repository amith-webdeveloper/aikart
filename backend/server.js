...
app.post("/api/chat", async (req, res) => {
  ...
  // --------------------------------------------------
  // DETERMINISTIC EXPLICIT CART ADD
  // --------------------------------------------------
  //
  // If the customer explicitly asks to add a product,
  // resolve and mutate the cart directly.
  //
  // Do NOT let the LLM decide the product ID,
  // quantity, or whether the mutation should happen.
  // --------------------------------------------------

  if (wantsAddToCart && addToCartMatch) {
    const requestedProductName =
      addToCartMatch[3].trim();

    console.log(
      "Deterministic cart add request:",
      {
        productName: requestedProductName,
    ...
    console.log(
      "Deterministic cart add resolution:"
    );
    console.dir(
      resolutionResult,
      { depth: null }
    ...
    if (!addResult.success) {
      return sendChatResponse(
        ...
        userMessage,
        addResult.error ||
        `I couldn't add ${resolutionResult.productName} to your cart.`
      );
    }
    ...
    return sendChatResponse(
      ...
      session,
      userMessage,
      `${resolutionResult.productName} has been added to your cart.`
    );
  }

  // --------------------------------------------------
  // DETERMINISTIC EXPLICIT CART REMOVE
  // --------------------------------------------------
  //
  // Explicit removals are also executed directly.
  // Never trust a product ID generated by the LLM.
  // --------------------------------------------------

  const removeFromCartMatch =
    userMessage.match(
      /\b(?:remove|delete|take)\s+(.+?)\s+(?:from|out of)\s+(?:my\s+)?(?:cart|basket)\b/i
    );

  const wantsRemoveFromCart =
    isExplicitCartRemoveRequest(userMessage);

  if (wantsRemoveFromCart && removeFromCartMatch) {
    const requestedProductName =
      removeFromCartMatch[1].trim();

    console.log(
      "Deterministic cart remove request:",
      {
        productName: requestedProductName,
      }
    );
    ...
    console.log(
      "Deterministic cart remove resolution:"
    );
    console.dir(
      resolutionResult,
      { depth: null }
    ...
    console.log(
      "Deterministic cart remove result:"
    );
    console.dir(
      removeResult,
      { depth: null }
    ...
    return sendChatResponse(
      ...
      session,
      userMessage,
      `${resolutionResult.productName} has been removed from your cart.`
    );
  }


  // --------------------------------------------------
  // DETERMINISTIC CART VIEW
  // --------------------------------------------------
  //
  // Showing the cart is a read-only application-state operation.
  // Do not rely on the LLM to decide or describe cart contents.
  // --------------------------------------------------

  const wantsToViewCart =
    /\b(show|view|see|display|check)\b.*\b(my\s+)?(cart|basket)\b/i.test(
      userMessage
    );

  if (wantsToViewCart) {
    console.log("Deterministic cart view request.");

    const cartItems = getCart(session.cart);

    console.log("Deterministic cart view result:");
    ...
    return sendChatResponse(
      ...
      session,
      userMessage,
      formatCartForCustomer(cartItems)
    );
  }
...
`,
    ...
    while (true) {
      ...
      // --------------------------------------------------
      // NORMALIZE TEXT-FORMAT TOOL CALLS
      // --------------------------------------------------
      //
      // Some Qwen responses return tool calls inside
      // assistantMessage.content instead of the structured
      // tool_calls field. Convert that format into the same
      // structure used by the existing tool execution loop.
      // --------------------------------------------------
      ...
      if (
        ...
        const toolCallMatch =
          assistantMessage.content.match(
        ...
        if (toolCallMatch) {
          try {
            ...
            if (
              ...
              console.log(
                "Normalized text-format tool call:"
              );
              console.dir(
                assistantMessage.tool_calls,
                { depth: null }
            ...
            console.log(
              "Failed to parse text-format tool call:",
              error.message
            );
          }
        }
          ...
          if (
            ...
            if (addResult.success) {
              ...
              return sendChatResponse(
                ...
                session,
                userMessage,
                `${toolResult.productName} has been added to your cart.`
              );
            }
